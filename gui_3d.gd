...
export (PackedScene) var gui_scene = preload("res://GUI.tscn")
...
func _ready():
	...
	#gui.set_owner(self)
	gui.set_name("gui")
	get_node(NodePath("./Viewport")).add_child(gui)
...
func _input(event):
	if not Globals.menu:
		...
		if is_mouse_event and not Globals.move_locked() and (is_mouse_inside or is_mouse_held):
			handle_mouse(event)
		elif not is_mouse_event and keyboard_focus:
...
func on_keyboard_focus_entered():
	keyboard_focus = true


func on_keyboard_focus_exited():
	keyboard_focus = false

func _exit_tree():
	Globals.focusAbles.erase(gui)
	Globals.gui_3ds.erase(self)
